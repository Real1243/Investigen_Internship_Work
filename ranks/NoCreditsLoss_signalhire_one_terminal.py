...
# We dont lose credits on this one but all the details are shown but takers indefinite time -->
...
# Paste your CURRENT ngrok https URL here every time you run
CALLBACK_URL = "https://your-ngrok-url.ngrok-free.app"  # ← CHANGE THIS!

app = Flask(__name__)
results_cache = {}  # name → details
pending_requests = set()  # to avoid duplicate requests for same person
...
def reveal_single_person(uid):
    if not uid: return
    headers = {"apikey": API_KEY, "Content-Type": "application/json"}
    payload = {"items": [uid], "callbackUrl": CALLBACK_URL}
    resp = requests.post(BASE_URL + PERSON_ENDPOINT, headers=headers, json=payload)
    print(f"Request sent for UID {uid} → Status: {resp.status_code} | {resp.text}")
    print("Waiting for callback... details will appear automatically when ready.\n")
...
def callback():
    ...
    try:
        ...
        for item in items:
            ...
            print(f"✅ DETAILS READY FOR: {name}")
            print("   → Type the number again to view full info!\n")
    except Exception as e:
        print("Callback error:", str(e))
    return {"status":"ok"}, 200
...
if __name__ == "__main__":
    ...
    print("\nSignalHire Enricher - One-by-One Mode (Credit-Safe)")
    print("Note: Credits only used when you select a person and contacts are found.")
    
    company = input("\nEnter company name (e.g., HDFC Bank): ").strip()
    
    profiles = search_finance_treasury(company)
    if not profiles:
        print("No results.")
    else:
        print(f"\nTop 10 most senior for {company}:\n")
        person_map = {}
        uid_map = {}  # number → uid
        for i, p in enumerate(profiles, 1):
            name = p.get("fullName", "N/A")
        ...
        print("\nTo get contacts for a person:")
        print("Type their number (1-10) → request will be sent for that person only.")
        print("Type 'exit' to quit.\n")
        
        while True:
            inp = input("Enter person number (1-10) or 'exit': ").strip()
            if inp.lower() == 'exit':
                print("Exiting.")
                break
            if not inp.isdigit() or int(inp) not in person_map:
                print("Invalid! Choose 1-10 or 'exit'")
                continue
            ...
            if name in results_cache:
                ...
                print(f"  Started : {d['start']}")
                print("\nWork emails:")
                print("  " + "\n  ".join(d['emails']) if d['emails'] else "None found")
                print("\nPhones:")
                print("  " + "\n  ".join(d['phones']) if d['phones'] else "None found")
                print("─"*70)
            elif uid and uid not in pending_requests:
                pending_requests.add(uid)
                print("Sending request for this person only (1 credit if successful)...")
                reveal_single_person(uid)
                print("Waiting for callback... Try typing the number again in 30-90 seconds.\n")
            else:
                print("No UID or already requested. Wait or try another.\n")